...
DROP POLICY IF EXISTS "Enable read access for all" ON daily_workouts;
DROP POLICY IF EXISTS "Enable read access for all" ON workout_exercises;
DROP POLICY IF EXISTS "Enable read access for all" ON exercise_sets;
...
BEGIN
  ...
  WITH workout_stats AS (
    ...
    LEFT JOIN exercise_sets es ON es.exercise_id = we.id AND es.user_id = d.user_id
